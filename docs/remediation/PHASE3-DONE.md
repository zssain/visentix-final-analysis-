...
| FIND-001 | **Governed — left UNCHANGED.** `../../logs/archive/2026-07/DECISION-NEEDED.md` §2.7 parks `enforcement_matches` as an SME decision and ends "STOP — awaiting approval"; no spec resolves it. Behaviour not changed; exact approval + spec-update path flagged in `logs/decision-log.md`. | `logs/decision-log.md` (flag only) | — | **BLOCKED — SME APPROVAL** |
