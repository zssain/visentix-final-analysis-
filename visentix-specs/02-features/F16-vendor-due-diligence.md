...
## Changelog
- 2026-07-16 (audit): AC-2 banned-term unit test implemented (`guardrails.test.ts`); signal codes now render via CodexTooltip (DDR-006); decision panel refactored to a keyed subcomponent (removes a setState-in-effect lint error; no behavior change). Mock codes flagged as placeholders (Hard Rule 3).
