...
## API contracts
- `GET /review/queue` — assessments awaiting review, **each identified by what it is**: organization name/domain/industry, what was submitted (URL, uploaded filename, or pasted text), capture date, and decided/total finding counts. Enrichment is read-only from existing rows and **degrades to no labels rather than failing the queue**. A bare UUID is not an item name (DDR-011).
...
- AC-10 A queue row is labelled by its organization and submitted source, with decided/total findings; the assessment id is reachable (hover/detail header) but is never the row's label. A row whose organization is unrecorded says so.
