...
## Data
Writes: `assessment`, `privacy_notice`, `notice_section`, `disclosure_clause` (with embeddings + LLM classification), and `assessment_intake_scope` (migration 0048). `privacy_notice` records intake provenance: `intake_method` (url/text/upload) and — for uploads only — `upload_filename`, `upload_mime`, `upload_file_hash` (sha256 of the original bytes; `content_hash` remains the hash of the extracted text). `assessment_intake_scope` freezes optional user-declared organization size/status/geography, state footprint, selected-law scope, data categories, business practices, and per-field provenance without rewriting corpus clauses. Reads: `clause_taxonomy`. (Migrations 0033, 0048.)
...
- Optional scope flow: organization profile → state footprint and separately selected-law scope → data categories → business practices → review and confirm. All option vocabularies come from the backend's governed profiling config. URL-only submission remains supported. Async submissions are shown in a client queue whose status is read only from `GET /assessments/:id/status`.
...
- AC-13 State footprint and selected-law scope persist as distinct values; every optional scope field records user-declared vs inferred/absent provenance and is frozen into the report snapshot.
- AC-14 Intake presents a review/confirm step, states that the deliverable includes an on-screen report and shareable PDF, and tracks multiple async submissions with server-authoritative status.
...
## Changelog
- 2026-08-21 (engineer): **QA intake scope remediation.** Added governed profile/data/practice inputs, distinct footprint vs selected-law scope, review/confirm, multi-job async queue, immutable `assessment_intake_scope` persistence (0048), report-scope provenance, and AC-13/14. URL-only intake remains supported; declared categories/practices are recorded but do not alter formulas without an expert-approved formula version.
