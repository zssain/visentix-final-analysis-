...
## Changelog
- 2026-07-16 (audit): Finding codes in §7 and bulk evidence now render via CodexTooltip (DDR-006 hover/focus target, navy code-chip); regulator heatmap QoQ deltas made judgement-neutral (counts, not verdicts); AC-8 polarity unit tests + banned-term scans over M-15–M-24 data implemented in `web/src/test/guardrails.test.ts`. Mock finding codes flagged as placeholders pending the real catalog (Hard Rule 3).
