...
| **Shell & primary navigation** (DDR-010) | Grouped left rail. Active state is unmistakable (filled pill + left rule); grouping is carried by **spacing and a divider rather than a shouted label**; icons are quiet and consistently weighted; generous vertical rhythm. Below 900px it collapses to a hamburger drawer (unchanged). On desktop the rail can be **hidden entirely on demand** (owner-decided 2026-09-01): a hide control in the rail header, a floating restore control where the rail's edge was, Ctrl/Cmd+B, preference persisted per browser. Hidden or shown are the only two states — the icon-only collapsed mode stays rejected. The rail is chrome — it never competes with the content for attention |
