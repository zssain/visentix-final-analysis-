...
- **Enforcement:** the banned-term filter runs on every narrative output; LLM rephrasing is verified with deterministic fallback; every deliverable opens with a plain-language **scope statement** and closes with a single **Disclosure** block, per the revised DDR-007 (design-system §3/§4). The per-surface "Intelligence, not legal advice" mark it replaces is retired **only once both bookends actually render** — dropping the mark without them is a regression. *Revision owner-decided 2026-08-31; the code change needs expert + engineer joint approval.*
- **No negative framing.** Beyond the verdict ban, customer-facing prose describes position and opportunity rather than deficiency in the reader: what stronger peers disclose, and what closing the gap would move. "Your notice lacks a retention period" becomes "most organizations like yours state a retention period; yours does not yet." This is a register rule, never a softening of what the evidence shows — a real gap is still stated plainly, and honest absence is never hidden behind pleasant wording.
- **Must vs should — modality is carried, never assigned.** "Must" and "should" are not interchangeable and must not be flattened: **must** reflects a *required* disclosure under a cited source; **should** reflects *recommended practice* (peer norm, regulator guidance, or a Visentix recommendation). Every requirement-flavored statement therefore renders its modality explicitly and names where the modality came from — the cited obligation's own language for "must", the exemplar cohort or authored recommendation library for "should". Visentix **never upgrades** a should into a must, and never asserts a must without a citable source carrying that modality; where the source modality is unrecorded, the statement renders as "should" with its provenance shown. Guardrail note: "required under [source]" is a description of the source, not a finding about the organization — "you must do X" is a verdict and stays banned. **The authoritative source of modality per obligation is not yet governed — see OD-16; until it closes, no surface may display a "must".**
...
## 10. Changelog
- 1.3 (2026-08-31): **Owner feedback pass — §2 guardrail register.** Replaced the per-surface "Intelligence, not legal advice" mark with **scope up front + one closing Disclosure** (revised DDR-007; retirement contingent on both bookends rendering; expert + engineer joint approval required before code). Added the **no-negative-framing** register rule and the **must-vs-should modality** rule (modality is carried from a citable source, never assigned or upgraded; blocked on OD-16 before any "must" ships). The banned-term list, required vocabulary, approved-alternative mapping, VCI routing, and source-excerpt exception are **unchanged** — nothing was softened. Source: owner (product) verbal notes.
